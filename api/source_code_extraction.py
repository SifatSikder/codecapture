...
class CodeExtractor:
    ...
    def extract_sidebar(self,model_path,image_path,image_name,save_path):
    ...
    def extract_components(self,image_folder_path,model_path):
        model = YOLO(model_path)
        classes = model.names
    ...
    def rectify_code_structures(self,extracted_code_structures):
    ...
    def extract_sidebars_files_codes_codes_structures(self,response):
        ...
        rectified_code_structures = self.rectify_code_structures(extracted_code_structures)
    ...
    def merge_all_json(self,root_dir):
    ...
    def compile_code(self,video_texts):
        sidebar_pattern = r'\\sidebar\s*\{(\s*\[.*?\]\s*)\}'
        active_files_pattern = r'\\activeFiles\s*\{(\s*\[.*?\]\s*)\}'
        codes_pattern = r'\\codes\s*\{(\s*\[.*?\]\s*)\}'
        code_structures_pattern = r'\\code_structures\s*\{(\s*\[.*?\]\s*)\}'
    ...
    def create_hierarchy_json(self,video_name,extracted_sidebar):
    ...
    def create_code_json(self,video_name,extracted_rectified_code_structures):
    ...
    def hierarchy_and_code_json_generation (self,ocr_folder):
        for folder in os.listdir(ocr_folder):
            ...
            for file in os.listdir(os.path.join(ocr_folder,folder)):
                ...
                frame_text=self.read_json_text(file_path)
                video_texts.append(frame_text)
            video_name = folder
            response = self.compile_code(video_texts)
            extracted_sidebar, extracted_active_files, extracted_codes, extracted_code_structures,extracted_rectified_code_structures = self.extract_sidebars_files_codes_codes_structures(response)
            self.create_hierarchy_json(video_name,extracted_sidebar)
            self.create_code_json(video_name,extracted_rectified_code_structures)
    ...
    def add_long_path_prefix(self,path):
        if os.name == 'nt': return f"\\\\?\\{os.path.abspath(path)}"
        return path

    def create_structure(self,item, parent_path):
        full_path = os.path.join(parent_path, item['text'])
        if len(os.path.abspath(full_path)) >= 260: full_path = self.add_long_path_prefix(full_path)
        if item['type'] == 'folder': os.makedirs(full_path, exist_ok=True)
        elif item['type'] == 'file':
            with open(full_path, 'w') as file: pass
        return full_path

    def fix_json(self,raw_data):
        genai.configure(api_key="AIzaSyDJn8iLuEjbgsbVMog32wdEkWLSELH6-Q4")
        generation_config = {"temperature": 0.7,"top_p": 0.6,"top_k": 40,"max_output_tokens": 20*1024,"response_mime_type": "text/plain",}
        model = genai.GenerativeModel(model_name="gemini-1.5-flash",generation_config=generation_config)
        chat_session = model.start_chat(history=[])
    ...
    def create_hierarchy_from_json(self,json_file):
        ...
        with open(json_file, 'r') as file:
            raw_data = file.read()
            cleaned_list = self.fix_json(raw_data)
            cleaned_list = json.loads(cleaned_list)
            id_to_path = {}
            for i,item in enumerate(cleaned_list):
                if item['parent_id'] is None:
                    path = self.create_structure(item, base_folder)
                else:
                    parent_path = id_to_path[item['parent_id']]
                    path = self.create_structure(item, parent_path)
    ...
    def create_code_from_json(self,json_file):
        os.makedirs("individual_results",exist_ok=True)
        base_name = os.path.splitext(os.path.basename(json_file))[0]
        base_folder = f"individual_results/{base_name}"
        os.makedirs(base_folder,exist_ok=True)
    ...
    def create_hierarchies(self,hierarchy_folder):
        for root, _, files in os.walk(hierarchy_folder):
            for file in files:
                file_path = os.path.join(root, file)
                self.create_hierarchy_from_json(file_path)

    def create_codes(self,code_folder):
        for root, _, files in os.walk(code_folder):
            for file in files:
                file_path = os.path.join(root, file)
                self.create_code_from_json(file_path)

    def copy_everything_from_source_to_destination(self,source, destination):
    ...
    def replace_matching_files(self,hierarchy_code_paths):
    ...
    def hierarchies_with_codes(self,final_results_folder):
            ...
            if os.path.isdir(video_path):
                ...
                for folder_inside_video in os.listdir(video_path):
                    if folder_inside_video == 'hierarchy':
                        ...
                        self.copy_everything_from_source_to_destination(hierarchy_path, hierarchy_with_code_path) 
        ...
        self.replace_matching_files(hierarchy_code_paths)            

    def create_merged_hierarchy_json(self,merged_hierarchy):
        file_path = os.path.join("merged_results", "merged_hierarchy.json")
        with open(file_path, "w", encoding="utf-8") as json_file:
            json.dump(merged_hierarchy, json_file, indent=4)

    def create_merged_hierarchies_json(self,hierarchy_folder):
        hierarchies = []
        ...
        if mergeability:
            ...
            self.create_merged_hierarchy_json(merged_hierarchy)
    ...
    def create_merged_hierarchies(self,merged_hierarchy_json_folder):
    ...
    def create_merge_version(self,versions):
    ...
    def create_merged_codes(self,code_folder):
        os.makedirs("merged_results",exist_ok=True)
        ...
        for filename, versions in repeated_active_file_dict.items():
            ...
            merged_version = self.create_merge_version(versions)
            destination_path = f"merged_results/merged_codes/{filename}"
            with open(destination_path, 'w', encoding='utf-8') as f:
                f.write(merged_version)
                
    def create_merged_hierarchies_with_codes(self,merged_hierarchy_json_folder):
        ...
        self.replace_matching_files([hierarchy_code_path])
